...
namespace DTcms.Web.tools
{
    ...
    public class submit_ajax : IHttpHandler, IRequiresSessionState
    {
        ...
        private void user_invite_code(HttpContext context)
        {
            ...
            {
                int result = codeBll.GetCount("user_name='" + model.user_name + "' and [type]='" + DTEnums.CodeEnum.Register.ToString() + "' and datediff(\"d\",add_time,now())=0");
            ...
            //删除过期的邀请码
            codeBll.Delete("[type]='" + DTEnums.CodeEnum.Register.ToString() + "' and (status=1 or datediff(\"d\",eff_time,now())>0)");
            //随机取得邀请码
        ...
        private void user_info_edit(HttpContext context)
        {
            ...
            string safe_question = DTRequest.GetFormString("txtSafeQuestion");
            string safe_answer = DTRequest.GetFormString("txtSafeAnswer");
            //检查邮箱
            if (nick_name == "")
            {
