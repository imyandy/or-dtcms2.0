...
namespace DTcms.DAL
{
    ...
    public partial class users
    {
        ...
        {
            string strSql = "select top 1 id from dt_users order by id desc";
        ...
        public bool Exists(string reg_ip, int regctrl)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(1) from dt_users");
            strSql.Append(" where reg_ip=@reg_ip and DATEDIFF(hh,reg_time,now())<@regctrl ");
            OleDbParameter[] parameters = {
                ...
                using (OleDbTransaction trans = conn.BeginTransaction())
                {
                    try
                    {
                        ...
                        OleDbParameter[] parameters = {
					            ...
					            new OleDbParameter("@qq", OleDbType.VarChar,30),
					            new OleDbParameter("@address", OleDbType.VarChar,255),
					            new OleDbParameter("@safe_question", OleDbType.VarChar,255),
					            new OleDbParameter("@safe_answer", OleDbType.VarChar,255),
